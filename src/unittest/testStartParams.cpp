...
TEST(testArgumentParsing, noArguments) {
	...
	EXPECT_ANY_THROW(BT::CStartParams(getVectorOfString(args)));
}
...
TEST(testArgumentParsing, helpNotRequested) {
	...
	auto const& config = BT::CStartParams(getVectorOfString(args));
...
TEST(testArgumentParsing, invalidPeerIP) {
	...
	EXPECT_ANY_THROW(BT::CStartParams(getVectorOfString(args)).getPeers());
}
...
TEST(testArgumentParsing, noExceptionWhenHelpIsRequested) {
	...
	EXPECT_NO_THROW(BT::CStartParams(getVectorOfString(args)));
}
#endif