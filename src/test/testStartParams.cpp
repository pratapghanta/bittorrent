...
TEST(testArgumentParsing, noArguments) {
	...
	EXPECT_ANY_THROW(BT::StartParams_t(getVectorOfString(args)));
}
...
TEST(testArgumentParsing, helpNotRequested) {
	...
	auto const& config = BT::StartParams_t(getVectorOfString(args));
...
TEST(testArgumentParsing, invalidPeerIP) {
	...
	EXPECT_ANY_THROW(BT::StartParams_t(getVectorOfString(args)).getPeers());
}
...
TEST(testArgumentParsing, noExceptionWhenHelpIsRequested) {
	...
	EXPECT_NO_THROW(BT::StartParams_t(getVectorOfString(args)));
}
#endif